      ...
      {clients.map((client) => {
        ...
        return (
          <button
            ...
            className={`
              relative flex items-center gap-1.5 px-1 py-0.5 rounded transition-all duration-300 group
              ${isSelected 
                ? 'opacity-100' 
            ...
            <div className="relative z-10 flex items-center gap-1.5 px-1 py-0.5">
              {client.logo && (
                <div className="shrink-0">
                  <ClientLogo logo={client.logo} size={16} />
                </div>
              )}
              <span className={`
                text-sm font-normal leading-[18.2px] tracking-[-0.28px] whitespace-nowrap transition-colors duration-300
                ${isSelected 
                  ? 'text-[#F1F2F3]' 
                  : 'text-[#F1F2F3]'
                }
